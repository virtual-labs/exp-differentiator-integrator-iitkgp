...
//---------------------------------------------------------Square wave-------------------------------------------------------------------------------//
   ...
   var x1 = new Array(), y1 = new Array();  // x,y plotting variables
    ...
    var x2 = new Array(), y2 = new Array(), y3 = new Array();
   var j;
    // define plot paramaters
    tstart1 = 0;//-tmaxs; //in sec
    tstop1 = tmaxs;
...
for (j = 0; j < 101; j++) {
        ...
        var p = 2 * 3.1415 * frqfng * x1[j] + phsl * 3.1415 / 180;
        var z = Math.sin(p);
        y1[j] = (parseFloat(vp / 3) * Math.asin(0 - z)); //-Math.sign(Vp *Math.sin(2 * 3.1415 * fo * x[i] + phase * 3.1415 / 180));
    }
    for (j = 101; j< 202; j++) {
        x1[j] = (tstart1 + j * dt1).toPrecision(6);
        var r = 2 * 3.1415 * frqfng * x1[j] + phsl * 3.1415 / 180;
        var s = Math.sin(r);
        y2[j] = (parseFloat(vp / 3) * Math.asin(s)); // Math.sign(Vp *Math.sin(2 * 3.1415 * fo * x[i] + phase * 3.1415 / 180));
    }
    for (j = 0; j < axes.N; j++) {
        y3[j] = (parseFloat(y1[j]) - parseFloat(y2[j + 101]));
    }
